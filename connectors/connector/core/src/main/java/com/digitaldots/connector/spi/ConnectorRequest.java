...
/**
 * <p>
 * A connector request. The request opens an interaction with the connector. Because a request represents a single interaction, the request
 ...
 *
 * <p>
 * Optionally, a connector may also extend the request interface and provide dedicated (type-safe) methods for configuring a request,
 * preferably in a fluent API fashion:
 * 
 * <pre>
 * connector.createRequest().endpointUrl("http://mysystem.loc/foo").payload("some important payload").execute();
 * </pre>
 * 
 ...
 *
 * <p>
 * A request must return a {@link ConnectorResponse} that contains response data. Requests for which the response contains no payload or is
 * not relevant should return an empty response that provides no data.
 * </p>
 *
 *
 */
public interface ConnectorRequest<R extends ConnectorResponse> {
    ...
    default void setAuthentication(Authentication authentication) {
    }
